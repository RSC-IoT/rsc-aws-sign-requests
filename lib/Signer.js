...
const encrypt = (key, src, encoding) => {
...
const canonical_request = (request) => {
    let url_info = url.parse(request.url);
    return [
        request.method || '/',
        url_info.path,
        url_info.query,
...
const parse_service_info = (request) => {
    ...
    let matched = host.match(/([^\.]+)\.(?:([^\.]*)\.)?amazonaws\.com$/);
 ...
 */
const get_authorization_header = (algorithm, access_key, scope, signed_headers, signature) => {
    return [
 ...
 */
module.exports = (request, access_info, service_info) => {
    if (service_info === void 0) service_info = null;
    request.headers = request.headers || {};

    // datetime string and date string
    let dt = new Date(), dt_str = dt.toISOString().replace(/[:\-]|\.\d{3}/g, '');
    let d_str = dt_str.substr(0, 8), algorithm = 'AWS4-HMAC-SHA256';
    let url_info = url.parse(request.url);
    ...
    let request_str = canonical_request(request);

    // Task 2: Create a String to Sign
    service_info = service_info || parse_service_info(request);
    let scope = credential_scope(d_str, service_info.region, service_info.service);
    let str_to_sign = string_to_sign(algorithm, request_str, dt_str, scope);

    // Task 3: Calculate the Signature
    let signing_key = get_signing_key(access_info.secret_key, d_str, service_info);
    let signature = get_signature(signing_key, str_to_sign);

    // Task 4: Adding the Signing information to the Request
    let authorization_header = get_authorization_header(algorithm, access_info.access_key, scope, signed_headers(request.headers), signature);
